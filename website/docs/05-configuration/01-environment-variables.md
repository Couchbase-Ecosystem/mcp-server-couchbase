...
| `CB_MCP_READ_ONLY_MODE` | `--read-only-mode` | Prevent all data modifications (KV and Query). See [Read-Only Mode](./05-configuration/02-read-only-mode.md) for details. | `true` |
...
| `CB_MCP_DISABLED_TOOLS` | `--disabled-tools` | Tools to disable (see [Disabling Tools](./05-configuration/04-disabling-tools.md)) | None |
| `CB_MCP_CONFIRMATION_REQUIRED` | `--confirmation-required` | Tools requiring user confirmation before execution (see [Elicitation/Confirmation for Tool Calls](./05-configuration/05-elicitation-for-tools.md)) | None |
