...
class TestValidateLogLevel:
    ...
    def test_valid_level_returns_tuple_with_none_invalid(self):
        resolved, invalid = validate_log_level(None, None, "DEBUG")
        assert resolved == "DEBUG"
        assert invalid is None

    def test_invalid_level_returns_default_plus_original_token(self):
        resolved, invalid = validate_log_level(None, None, "BOGUS")
    ...
    def test_ignores_ctx_and_param_arguments(self):
        ...
        result = validate_log_level("sentinel-ctx", "sentinel-param", "INFO")
...
class TestValidateLogSinks:
    ...
    def test_valid_sinks(self):
        sinks, invalid = validate_log_sinks(None, None, "stderr,file")
        assert sinks == {"stderr", "file"}
        assert invalid == []

    def test_invalid_token_collected(self):
        sinks, invalid = validate_log_sinks(None, None, "stderr,bogus")
        assert sinks == {"stderr"}
        assert invalid == ["bogus"]

    def test_ignores_ctx_and_param_arguments(self):
        result = validate_log_sinks("sentinel-ctx", "sentinel-param", "stderr")
...
class TestValidateLogPath:
    ...
    def test_passes_non_empty_path_through(self):
        assert validate_log_path(None, None, "/tmp/foo.log") == "/tmp/foo.log"
