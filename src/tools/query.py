...
def _extract_query_from_agent_answer(raw_answer: str) -> str:
    """Extract SQL++ query text from model output that may include explanation."""
    ...
    block_matches = _SQL_BLOCK_RE.findall(answer)
...
def generate_or_modify_sql_plus_plus_query(  # noqa: PLR0911, PLR0912
    ...
    if not catalog_prompt:
        ...
        fallback_lines = [
            ...
            "Known scopes: "
            + (", ".join(scope_names) if scope_names else "Not available"),
            "Known collections: "
    ...
    try:
        resp_body = call_agent(
            content=message,
            # iQ expects these fields at top-level (sibling of "data"), not inside "data"
            extra_payload={
